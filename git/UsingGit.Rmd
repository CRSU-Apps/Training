...
knitr::include_graphics("images/bad_version_control.png")
```
...
Consider a to-do list which contains an item "Email Frank". One user has changed the item to "Email Helen" because Frank has moved onto another project, and added an extra item "Book bouncy castle", as in \@ref(fig:merge-conflict-first-user).

```{r, label = "merge-conflict-first-user", dpi = 50, fig.align = "center", echo = FALSE, fig.cap = "Changes made by the first user"}
knitr::include_graphics("images/complex_merge_first_user_changes.png")
```

After the first user's `r glossary("commit")` and `r glossary("push")` a second user had made changes without first `r glossary("pull", display = "pulling")` the first user's changes. They added 'about "Operation Falafel"' to the "Email Frank" item, to record what they are emailing about, and added an extra item "Submit paper abstract", as in \@ref(fig:merge-conflict-second-user).

```{r, label = "merge-conflict-second-user", dpi = 50, fig.align = "center", echo = FALSE, fig.cap = "Changes made by the second user"}
knitr::include_graphics("images/complex_merge_second_user_changes.png")
```

If the second user tries to `r glossary("commit")` and `r glossary("push")` their changes, they will be warned that their changes are out-of-date with respect to the `r glossary("remote")` `r glossary("repository", display = "repo")`. When they `r glossary("pull")` changes, they find that their `r glossary("local")` changes `r glossary("conflict")` with the `r glossary("remote")` changes, as in \@ref(fig:complex-conflict). Both sets of changes are valid, so they must be manually combined in order to keep all of the information. This is not something which can be done automatically; the manual changes must be done in an external editor, such as *Notepad++*. *`r glossary("Git")`* will mark the `r glossary("conflict", display = "conflicting")` changes with marker lines ('\<\<\< HEAD', '====' and '\>\>\>\> ...'), as in \@ref(fig:complex-conflict).

```{r, label = "complex-conflict", dpi = 50, fig.align = "center", echo = FALSE, fig.cap = "Example of a complex merge conflict"}
knitr::include_graphics("images/complex_merge_conflict.png")
```

In this case the "Email Frank" item can be combined to include both the new name, *and* the additional information. Both new items are also included in the final resolved file. Note that the marker lines must be manually removed. See the resolved `r glossary("conflict")` in \@ref(fig:complex-conflict-resolution).

```{r, label = "complex-conflict-resolution", dpi = 50, fig.align = "center", echo = FALSE, fig.cap = "Example of the resolved conflict"}
knitr::include_graphics("images/complex_merge_resolution.png")
```
...
#### Create the Branch {#sourcetree-create-branch}
