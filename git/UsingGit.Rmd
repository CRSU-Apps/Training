...
Consider a to-do list which contains an item "Email Frank". One user had added some context to the item to record what they are emailing about. A second users has changed the item to "Email Helen" because Frank has moved onto another project. Both users have also added an extra item to the list. When the first user comes to `r glossary("pull")` changes, they find that their changes conflict, as in \@ref(fig:complex-conflict). Both sets of changes are valid, so they must be manually combined in order to keep all of the information. This is not something which can be done automatically. The manual changes must be done in an external editor, such as *Notepad++*.
...
In this case the "email" line can be combined to include both the new name, *and* the additional information. Both new items are also included in the final resolved file. Note that the '<<< HEAD', '====' and '>>>> ...' lines must be manually removed. See the resolved conflict in \@ref(fig:complex-conflict-resolution).
